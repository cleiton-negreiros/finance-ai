...
import { getDB, saveDB } from './db-termux.js';
...
export async function importCSV(filePath, fonte) {
  ...
  const sql = `INSERT OR IGNORE INTO transacoes (fonte, conta, tipo, valor, moeda, descricao, categoria, data, hash)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

  let insertedCount = 0;

  for (const row of processed) {
    const existing = db.exec(`SELECT COUNT(*) as cnt FROM transacoes WHERE hash = '${row.hash}'`);
    const count = existing[0]?.values[0]?.[0] || 0;
    
    if (count === 0) {
      db.run(sql, [
        row.fonte, row.conta, row.tipo, row.valor, row.moeda,
        row.descricao, row.categoria, row.data, row.hash
      ]);
      insertedCount++;
    }
  }
